...
- Preserve reproducible observations using the [evidence collection guide](references/collection.md): dated HTTP snapshots or precisely identified local/supplied artifacts, linked to findings. The optional collector records responses; it does not render pages, interpret crawler permissions, or measure SEO outcomes.
- Complete the audit with the repository, accessible public pages, and earlier reports. This skill does not use Search Console or Bing Webmaster Tools; the audit stays scoped to what the site itself shows. Other private dashboards or analytics exports remain optional enrichment when the user already has them open or supplies them. Do not initiate sign-in/OAuth, request credentials, or wait for an account connection. Missing private data is a stated limit, not a defect or an automatic action to connect tools.
...
Use the [intent research method](references/intent-research.md) to connect visitor needs to existing pages and justified opportunities, even without keyword tools. When Perplexity or Claude is in scope, read the [provider guide](references/perplexity-claude.md); preserve search, user-retrieval, and training distinctions and flag conflicting documentation instead of guessing.
